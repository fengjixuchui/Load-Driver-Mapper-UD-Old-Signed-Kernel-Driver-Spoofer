...
{
	hDriver = CreateFile(skCrypt(L"\x5C\x5C\x2E\x5C\x4E\x61\x6C"), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, 0, OPEN_EXISTING, 0, 0);
	if (hDriver == INVALID_HANDLE_VALUE)
	{
		printf(skCrypt("\n\x20\x46\x61\x69\x6C\x65\x64\x20\x74\x6F\x20\x6F\x70\x65\x6E\x20\x48\x41\x4E\x44\x4C\x45"));
	}
}
...
uintptr_t KeInterface::getModuleExport(const char* module_name, uintptr_t module_base)
{
	...
	for (size_t i = 0; i < pExportDir->NumberOfNames; i++, pName++)
	{
		...
		if (!strcmp(module_name, pModuleName))
		{
			...
			{
				printf(skCrypt("\n\x20\x46\x6F\x72\x77\x61\x72\x64\x65\x72\x20\x45\x78\x70\x6F\x72\x74"));
